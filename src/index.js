...
(async () => {
  ...
  setInterval(() => {
    let text = `[${new Date().toLocaleTimeString()}] Channel Viewers Backend (ws:${PORT})\n\n`;
...
echoSocket.on("CV:Update", async ({ userIds, channelId, targetUserId, type }) => {
  io.sockets.sockets.forEach(socket => {
    if (userIds.includes(socket.data.id)) {
      socket.emit(type, [channelId, targetUserId]);
    }
  });
});
